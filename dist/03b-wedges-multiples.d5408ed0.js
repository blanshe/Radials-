...
var width = 1200 - margin.left - margin.right; // At the very least you'll need scales, and
...
var radius = 100;
var colorScale = d3.scaleLinear().range(['purple', 'pink']);
var pointScale = d3.scalePoint().range([radius, width - radius]);
...
function ready(datapoints) {
  ...
  var cities = datapoints.map(function (d) {
    return d.city;
  });
  ...
  console.log(nested);
  svg.selectAll('.polar-bar').data(nested).enter().append('g').each(function (d) {
    var container = d3.select(this);
    var datapoints = d.values;
    container.selectAll('.polar-bar').data(datapoints).enter().append('path').attr('d', arc).attr('fill', function (d) {
      return colorScale(+d.high_temp);
    }).attr('transform', function (d) {
      console.log(pointScale(d.city));
      return "translate(".concat(pointScale(d.city), ",").concat(height / 2, ")");
    });
    container.selectAll('.circle').data(datapoints).append('circle').attr('r', 3).attr('cx', 0).attr('cy', 0).attr('fill', 'black').attr('transform', function (d) {
      console.log(pointScale(d.city));
      return "translate(".concat(pointScale(d.city), ",").concat(height / 2, ")");
    });
    container.selectAll('.label').data(datapoints).enter().append('text').text(function (d) {
      return d.city;
    }).attr('text-anchor', 'middle').style('font-size', '20').attr('transform', function (d) {
      console.log(pointScale(d.city));
      return "translate(".concat(pointScale(d.city), ",").concat(height, ")");
    });
  });
}
