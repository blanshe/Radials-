...
var svg = d3.select('#chart-4').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').append('g').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")"));
var radius = 150;
var colorScale = d3.scaleLinear().range(['blue', 'pink']);
var radiusScale = d3.scaleLinear().range([radius / 2, radius]);
var angleScale = d3 // .scalePoint()
// .padding(0.5)
.scaleBand().range([0, Math.PI * 2]);
...
function ready(datapoints) {
  ...
  var months = datapoints.map(function (d) {
    return d.month_name;
  });
  angleScale.domain(months);
  svg.selectAll('polar-bar').data(datapoints).enter().append('path').attr('d', arc).attr('fill', 'pink');
  var bands = [20, 30, 40, 50, 60, 70, 80, 90];
  svg.selectAll('.band').data(bands).enter().append('circle').attr('r', function (d) {
    return radiusScale(d);
  }).attr('fill', 'none').attr('stroke', 'lightgray').lower();
  var bandsText = [30, 50, 70, 90];
  svg.selectAll('.amount-chart').data(bandsText).enter().append('text').text(function (d) {
    return d + '°';
  }).attr('y', function (d) {
    return -radiusScale(d);
  }).attr('dy', -8).attr('alignment-baseline', 'middle').attr('text-anchor', 'middle');
  svg.append('text').style('font-size', '24').attr('text-anchor', 'middle').text('NYC').attr('x', 0).attr('y', 0);
}
