...
var width = 780 - margin.left - margin.right; // At the very least you'll need scales, and

var svg = d3.select('#chart-1').append('svg').attr('height', height + margin.top + margin.bottom).attr('width', width + margin.left + margin.right).append('g').attr('transform', 'translate(' + margin.left + ',' + margin.top + ')').attr('transform', "translate(".concat(width / 2, ",").concat(height / 2, ")")); // you'll need to read in the file. And you'll need
...
var colorScale = d3.scaleOrdinal().range(['#7fbf7b', '#af8dc3', '#f1a340']);
var angleScale = d3.pie().value(function (d) {
  return d.task;
});
var labelArc = d3.arc().innerRadius(0).outerRadius(410);
...
function ready(datapoints) {
  ...
  svg.selectAll('.outside-label').data(pie(datapoints)).enter().append('text').text(function (d) {
    return d.data.task;
