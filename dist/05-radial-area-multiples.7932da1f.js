...
function ready(datapoints) {
  ...
  svg.selectAll('.polar-bar').data(nested).enter().append('g').each(function (d) {
    ...
    container.selectAll('.band').data(bands).enter().append('circle').attr('r', function (d) {
      return radiusScale(d);
    }).attr('fill', 'none').attr('stroke', 'lightgray').lower().attr('transform', function (d) {
      console.log(pointScale(d.key));
      return "translate(".concat(pointScale(d.key), ",").concat(height / 2, ")");
