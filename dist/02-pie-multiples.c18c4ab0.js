...
function ready(datapoints) {
  ...
  var project = datapoints.map(function (d) {
    return d.project;
  });
  ...
  console.log(nested);
  svg.selectAll('.pie-chart').data(nested).enter().append('g').each(function (d) {
    ...
    container.selectAll('path').data(pie(datapoints)).enter().append('path').attr('d', function (d) {
      ...
      return "translate(".concat(pointScale(d.data.project), ",").concat(height / 2, ")");
    });
    container.selectAll('.label').data(pie(datapoints)).enter().append('text').text(function (d) {
      return d.data.project;
    }).attr('text-anchor', 'middle').style('font-size', '24').attr('transform', function (d) {
      console.log(pointScale(d.data.project));
      return "translate(".concat(pointScale(d.data.project), ",").concat(height, ")");
    });
  });
}
